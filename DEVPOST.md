...
We diagnose concussion almost entirely on what the injured person tells us, which is a
problem, because the people most likely to have one are the people most motivated to
say they're fine. They want to get back on the field, back to class, back to normal.
...
Vestibular/Ocular Motor Screening, is seven tasks and needs no equipment at all.
...
One more thing ended up shaping the entire design. Light sensitivity and motion
sensitivity are concussion symptoms, so an app for concussed people that throws a
bright animated interface at them is provoking the exact symptoms it's supposed to be
measuring.

## What it does

Baseline runs a VOMS-style screening in your browser and reads the results against your
own numbers.

1. **Prepare.** A symptom inventory taken *before* the tasks, because provocation means
   a change from your own starting point, not some absolute score.
...
3. **Findings.** What provoked symptoms, what the camera measured, a return-to-sport
   ...
   flags that mean go to an emergency department right now, and a printable summary you
   can hand a clinician.

It measures things, which is what pushes it past being a questionnaire. Near point of
convergence comes out in actual centimetres, and that works because the human iris is
about 11.7 mm across in every adult, so iris-width-in-pixels becomes an absolute
distance reference. Five of the seven tasks also produce a pursuit gain, a phase lag,
or a vestibulo-ocular reflex gain.

It also remembers. "Baseline" is a clinical term long before it's a product name: you
...
Nothing leaves your device, because there's no account, no upload and no server. Your
record sits in your browser, with export and erase buttons right next to it.

## How I built it

It's React 19 with TypeScript, Vite and Tailwind, and it ships as a fully static site.
...
origin, so no runtime request goes to Google or anyone else. This is the part most
...
comparison, and the oculomotor maths. 116 tests cover it, and the components on top are
thin glue.

For the design, I built two rooms. The landing is loud, with a didone display face on
blush paper, grain, an editorial grid. Walking into the instrument triggers an
announced transition into a dim, still, low-stimulus mode: grain off, motion off,
luminance capped, and the app says out loud what it's doing and why. You can skip it.
That threshold is the argument of the whole project, made physical.
...
reachable with a provoked count of zero. It was invisible in the source and completely
obvious in a screenshot. This is exactly the failure the whole product exists to avoid,
where "we didn't measure it," "we couldn't resolve it," and "we measured it and found
nothing" collapse into one sentence. Both the heading and the rationale now come from
the two counts separately, with regression tests holding them there.
...
when the number wouldn't mean anything, so it refuses on too few frames, too much
blinking, a target that didn't travel, a head that didn't turn, or a fit too loose to
trust. Every refusal states its reason and quotes the fit, because a blank space reads
as "normal," and a measurement built on six usable frames is worse than no measurement
at all. It'll get believed.
...
greyscale have to carry identical meaning. Underneath all of it sit 116 tests over
logic that genuinely earns them.

## What I learned

The interesting engineering in a health tool turns out to be mostly in what it declines
to say. The hard parts were the dead bands, the null-versus-zero distinctions, and
getting four separate error messages right, none of which is eye tracking.
...
And that rendering is a debugging technique. Three of the worst defects in this project
were a false clinical claim, an unreadable printout, and an invisible focus ring on the
primary control, and all three were invisible in source and immediately obvious in a
screenshot.
